        ...
        <div class="ms-panel-body">
            <form id="basic-form" method="post" action="{{ route('room.update', $room->id) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea id="summernote" rows="" cols="2" class="form-control  @error('description') is-invalid @enderror" name="description" value="{{ $room->description }}">{{ $room->description }}</textarea>
                    @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
                </div>
